...
class compraController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Filtrar por número de comprobante si se proporciona un término de búsqueda
        if ($request->has('search')) {
            $searchTerm = $request->input('search');
            $query->whereHas('comprobante', function ($query) use ($searchTerm) {
                $query->where('numero_comprobante', 'like', '%' . $searchTerm . '%');
            });
        }

        // Obtener los resultados después de aplicar los filtros
        $compras = $query->get();
